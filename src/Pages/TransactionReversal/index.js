...
const TransactionReversal = () => {
  ...
  const TransactionReversalClasses = useStyles();
  ...
  useEffect(() => {
    document.title = 'Transaction Reversal';
  },[]);
  ...
  useEffect(() => {
    if (TransactionReversalData.isError) {
        ...
        swal(
          <div>     
            <p>{TransactionReversalData["message"]}</p>
          </div>
        )  
  ...
  return (
    <Box className={TransactionReversalClasses.maindiv}>
      <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
        <Grid item xs={6}>
          <Box className={TransactionReversalClasses.boxDiv}>
